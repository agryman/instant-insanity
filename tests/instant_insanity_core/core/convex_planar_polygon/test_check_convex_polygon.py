import numpy as np
import pytest

from manim_cairo3d.convex_planar_polygon import check_convex_polygon


def test_valid_triangle():
    points = np.array([[0, 0], [1, 0], [0, 1]], dtype=np.float64)
    check_convex_polygon(points)


def test_valid_square():
    points = np.array([[0, 0], [1, 0], [1, 1], [0, 1]], dtype=np.float64)
    check_convex_polygon(points)


def test_valid_pentagon():
    points = np.array([[0, 0], [2, 0], [3, 1], [1, 3], [-1, 1]], dtype=np.float64)
    check_convex_polygon(points)


def test_too_few_points():
    points = np.array([[0, 0], [1, 1]], dtype=np.float64)
    with pytest.raises(ValueError, match='at least 3 points'):
        check_convex_polygon(points)


def test_self_intersecting_polygon():
    points = np.array([[0, 0], [2, 2], [0, 2], [2, 0]], dtype=np.float64)
    with pytest.raises(ValueError, match='Self-intersection'):
        check_convex_polygon(points)


def test_concave_polygon():
    points = np.array([[0, 0], [2, 0], [1, 1], [2, 2], [0, 2]], dtype=np.float64)
    with pytest.raises(ValueError, match='not convex'):
        check_convex_polygon(points)


def test_malformed_input_wrong_shape():
    points = np.array([[0, 0, 1], [1, 0, 1], [0, 1, 1]],dtype=np.float64)  # shape (3, 3)
    with pytest.raises(ValueError):
        check_convex_polygon(points)


def test_malformed_input_non_numeric():
    points = np.array([['a', 'b'], ['c', 'd'], ['e', 'f']])
    with pytest.raises(TypeError):
        check_convex_polygon(points)