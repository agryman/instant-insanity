...
"""
import numpy as np
from manim import Scene, tempconfig, Square, Dot, LEFT, RIGHT, UP, DOWN, RED, GREEN, BLUE, BLACK, ORIGIN, OUT
...
class PolygonToDotDemo(GridMixin, Scene):
    def construct(self):
        ...
        green_square: Square = blue_square.copy().set_fill(GREEN)
        tracked_green_square: TrackedPolygon = TrackedPolygon(green_square)
        self.add(tracked_green_square)

        animator: PolygonToDotAnimator = PolygonToDotAnimator(tracked_green_square, red_dot)
        animator.play(self, alpha=1.0, run_time=3.0)
