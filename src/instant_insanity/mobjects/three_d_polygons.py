...
class ThreeDPolygons(VGroup):
    ...
    depth sort. This means that thee directed graph on the set of polygons defined by relation
    *X is strictly behind Y* in the given projection is assumed to be acyclic.
    ...
    It is up to users of this class to encode and decode meaningful names to and from its corresponding  `PolygonId`.
    ...
    4. the depth sorter sorts the interpolated scene paths and returns them in an ordered dict
