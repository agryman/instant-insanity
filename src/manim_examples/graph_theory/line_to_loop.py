...
def cubic_edge(u_pt: Point3D, v_pt: Point3D,
               min_len: float = 1.0,
    ...
    """
    a: Point3D = np.array(u_pt, dtype=float)
    b: Point3D = np.array(v_pt, dtype=float)
    dvec: Vector3D = b - a
    d: float = float(np.linalg.norm(dvec))
    ...
    n: Vector3D = np.array([-u[1], u[0], 0.0])  # +90° in xy-plane
    m: Point3D = 0.5 * (a + b)

    # Blend parameter: 1 → straight; 0 → loop
    t: float = min(1.0, d / min_len)
    ...
    along: float = (2.0 * half / 3.0) * t     # equals d/3 when t=1

    # Perpendicular/loop component grows as endpoints meet
    loop_r: float = (min_len * 0.5) * loop_scale * (1.0 - t)
    ...
    c1: Point3D = e1 + along * u + loop_r * n
    c2: Point3D = e2 - along * u + loop_r * u
...
class LineToLoop(Scene):
    def construct(self):
        # Positions
        p1: Point3D = LEFT * 4 + DOWN * 1
        p2: Point3D = LEFT * 1 + UP * 2
        p3: Point3D = RIGHT * 2 + DOWN * 2
        p4: Point3D = RIGHT * 4 + UP * 1
        ...
        min_len: float = 1.0      # your requested minimum visible length for the edge
        bend_max: float = 1.2     # how “round” the loop gets at coincidence

        # Always redraw the cubic from current vertex positions
        edge: Mobject = always_redraw(lambda: cubic_edge(g[1].get_center(),
                                                g[2].get_center(),
                                                min_len=min_len))
